...
function Menu() {
  ...
  const [invokeRerender, setInvokeRerender] = useState(false);
  ...
  const isLinkActive = (props) => {
    return props.location.href.includes(props.href);
  }
    ...
    <nav className="h-10 md:h-24 my-10">
      <div className="h-10 md:h-24 my-10 flex place-content-between">
        ...
        <div className="md:hidden">
          <button
            className={classnames("menu h-1/2 w-1/2", {
          ...
          {menuItems.map((item, idx) => (
            <Link
              ...
              className={classnames(
                "mx-2 border-b-4 border-indigo-600 border-opacity-0 hover:border-opacity-100 active:font-semibold active:border-opacity-100",
                { "font-semibold border-opacity-100": item.isActive }
              )}
              getProps={obj => handleLinkProps(obj, item, idx)}
              key={idx}
            >
              {item.title}
            </Link>
          ...
          >
            <defs>
              <filter id="blur1" x="-10%" y="-10%" width="120%" height="120%">
                ...
                ></feBlend>
                <feGaussianBlur
                  stdDeviation={mobileRadius * 0.45}
          ...
          <div className="absolute top-36 flex flex-col gap-7 w-full items-center">
            ...
            {menuItems.map((item, idx) => (
              <Link
                onClick={handleMobileMenuToggle}
                to={item.slug}
                className={classnames("mx-2 text-4xl text-white  border-b-4 border-white-600 border-opacity-0 hover:border-opacity-100", { "font-semibold border-opacity-100": item.isActive })}
                getProps={obj => handleLinkProps(obj, item, idx)}
                key={idx}
              >
                {item.title}
